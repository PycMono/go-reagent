...
产品以“Agent 员工”为入口：客户登录自己的组织后先浏览员工列表，明确选择一位员工
再进入聊天；管理员在同一目录进入员工管理、训练和发布。员工表示一个逻辑 Agent，
不是登录用户、共享聊天会话或独立系统账号；同一岗位允许创建多个不同员工。
...
- `/agents` 为客户员工目录及登录后的默认落地页；未登录用户由宿主登录接入处理。
  单客户匿名部署仍只进入配置租户的普通目录，不因此获得管理员权限。页面展示本
  租户 enabled 且已有可用版本的员工，可搜索名字/职责、分页浏览；不跨租户汇总。
- 卡片包含头像、名字、岗位/职责简介和“开始聊天”，数据源为本租户 Agent；创建和
  筛选参数由 profile_code 改为 agent_id。default_agent_id 仅用于推荐展示，不自动
  替客户选择员工或直接发送消息；只有一个员工时也保留明确选择入口。
- 点击员工进入 `/chat?agent_id=<id>`，先展示该员工名字、职责、欢迎语和推荐问题；
  点击卡片不创建空会话，第一次发送消息时才创建绑定该 agent_id 的 Conversation。
  直接访问无 agent_id、无 conversation_id 的聊天入口返回员工目录；参数无效、员工
  不可用或不属于当前租户时显示明确错误和返回目录入口，不静默改用其他员工。
- `/chat?conversation_id=<id>` 从经过 tenant+user 鉴权的历史记录恢复固定员工；若同时
  传入不同的 agent_id 则拒绝。员工归档后历史仍可读但不能发送。点击“切换员工”返回
  目录，另建目标员工会话或由用户明确选择其历史会话，不迁移消息、附件或临时文件。
...
`workspaces/chat/AGENTS.md` 是通用模板基础，不是所有员工共同编辑的生产文件；内置
Profile 的身份规则和被引用 Skills 在 bootstrap 时组成各自独立的初始 Bundle。模板
后续修改不会影响已创建员工，普通聊天只加载已发布版本，未发布训练候选不向客户开放。
员工目录和普通聊天在 Phase 1B 交付，管理员完整工作台在 Phase 1D 交付。

### 9.4 管理员快捷换模型（Phase 1D）

`POST /api/v1/agents/:agentID/model-config-releases` 创建并激活一个新完整版本，不修改
历史版本，也不要求先创建训练聊天。请求包含 expected_row_version、base_version_id、
允许的 model_config、change_summary 和人工发布确认；没有模型变化时拒绝空发布。
只接受当前生产版本为基线，复用其 Git 文件树、工具策略和运行限制；按 §4.2 重新
计算联合摘要，并按 §7.3/§8.1 校验真实文件、模型能力、工具兼容性及 SecretRef，写入
新发布证据。整个流程使用同一 Agent 准入互斥、版本分配、文件准备和 CAS 发布事务。
新版本 source_training_session_id=NULL，发布人、时间和变更说明仍完整记录。

并发配置发布及活跃训练的运行操作互斥；没有运行操作的训练允许保留，模型发布成功
后其基线在下次准入变为 stale。CAS 冲突返回 409，不覆盖别人已发布的版本。重复旧
...
来源训练列在 0007 先 nullable，0010 再补 FK；初始版本和快捷模型配置发布来源 NULL，可重复。
...
2. Phase 1B：agents/agent_versions、模板迁移、身份接入、员工目录、明确选择后的聊天与会话 Runtime。
3. Phase 1C：第三张训练表、Candidate/Git checkpoint、校验、人工发布、回滚及恢复。
4. Phase 1D：管理员完整工作台、快捷换模型、迁移与备份演练、端到端验收。

本轮已授权按完善后的方案开展整体开发。每阶段编写和评审实施计划，完成本阶段验收
再进入下一阶段；Phase 1A 从已有计划评审和基线测试开始。自动评测、微调和分布式部署未来按真实需求另立
设计，不作为第一版验收条件。
...
- 客户先看到本组织员工目录，明确选择后进入聊天；首条消息才创建会话，直接访问
  无目标聊天回目录；刷新/深链接可恢复目标，跨租户目标及冲突参数被拒绝。
- 切换员工不迁移会话历史、附件或临时文件；历史会话始终恢复原员工，归档后只读；
  未发布训练不影响客户，修改公共模板不改已有员工。
- 快捷换模型复用文件树并创建新完整版本，重算摘要和证据；并发/重复旧 CAS 请求
  不产生第二次发布，旧版本不被改写，已有训练在发现新基线时转 stale。
