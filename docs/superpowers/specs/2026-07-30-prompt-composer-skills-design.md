...
- 所有工作区读取通过 `os.Root` 锚定；入口检查时必须为普通文件，显式符号链接会被忽略；即使发生并发替换，读取也不能逃逸工作区；
...
若根目录 `AGENTS.md` 在入口检查时是可读的普通文件，Composer 将其放入独立的“项目专属指南”区块；
缺失、不可读或检查时为符号链接则静默略过。读取同样通过 `os.Root` 锚定；并发替换最多改变工作区内的
读取目标，不能读取工作区外的文件。
