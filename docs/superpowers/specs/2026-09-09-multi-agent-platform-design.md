...
## 2026-09-10 用户追加的工作台布局

按用户提供的 `needs/img.png`，训练台左侧为当前候选的聊天测试，右侧为对话训练；右侧提供改动/发布与检查点标签页，顶部固定校验与发布入口。窄屏按聊天测试、训练上下排列。

新增管理员本人可用的 `POST /api/v1/training-sessions/:trainingID/preview`。试聊使用只读候选副本，没有文件写工具、exec、MCP 或生产业务工具；按当前候选配置运行并计入普通 Runtime 配额。历史仅保留在当前页面，既不写正式 Conversation，也不写训练作者历史。候选有进行中操作或 partial 改动时拒绝试聊，不扩张原来的发布权限。

实际交付与验证记录见 `../plans/2026-09-10-platform-completion-ledger.md`。
