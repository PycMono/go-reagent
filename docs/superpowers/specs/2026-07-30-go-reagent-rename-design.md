...
将项目从 `go-tiny-claw` 完整重命名为 `go-reagent`，并采用与 Git 远端一致的
...
- 将命令入口目录从 `cmd/claw` 移至 `cmd/reagent`，同步代码、测试、提示词和运行示例。
...
`go-tiny-claw`、`cmd/claw` 和项目专用 `/secure/claw` 示例路径；之后逐项检查仍包含
...
- 全仓不再出现 `go-tiny-claw` 或 `cmd/claw`。
