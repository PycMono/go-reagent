...
**Goal:** Deliver the administrator employee/training workbench, complete model-only releases, and a verified migration/backup/end-to-end acceptance workflow.
...
**Spec:** `docs/superpowers/specs/2026-09-04-agent-self-training-design.md`, §§8, 9.3, 9.4, 10.1 and 11. Prerequisite plans: `2026-09-09-agent-employees-phase-1b.md` and `2026-09-09-agent-training-phase-1c.md` in this directory. Begin after their reviewed acceptance gates.
...
- [ ] Render guarded admin pages and customer directory's management link only when server capability and role both permit. Deep links load scoped detail, never a guessed fallback employee. All state changes remain POST/PATCH; page GET never creates training or publication. Authentication redirect is supplied by host integration, not a fake login form.
...
### Task 3: Administrator employee catalog and detail workspace
...
- [ ] Add Node tests with mocked fetch: template presentation copied into draft without changing template; repeated submit disabled; same template can create multiple employees; empty/error states distinct; search/page cursor maintained; stale CAS reload does not overwrite entered changes; archive conflict shows active training link only if authorized.
...
- [ ] Detail shows employee presentation, current version and model, status, authorized training history and create-training action with expected Agent.row_version. Presentation edit changes only allowed fields; archive uses CAS and displays backend live-training conflict. Incomplete creation offers only supported recovery/archival actions; do not invent an unimplemented retry endpoint.
...
**Files:** Create `docs/agent-employees-operations.md`, `docs/agent-employees-phase-1d-evidence.md`, `cmd/agent-verify/main.go` and focused tests only if existing migration command cannot host read-only verification; extend `docs/agent-employees-migration.md`, `docs/agent-training-recovery.md` and config Go comments. No backup scheduler or generic storage platform.
...
- [ ] Run full product scenario: directory→choose employee→first message; create a second employee from same template; train one employee across multiple turns; safe partial checkpoint and repair; inspect diff/history; explicit validate/publish; ordinary next-turn version refresh; rollback; model-only release; expiry/cancel/reconnect; two tenants isolated. Confirm images/SSE/history rename/search/delete still function and no messages/files move on employee switch.
...
- [ ] §9.3 employee management, workbench, reconnect and history authorization: Tasks 2–4.
...
- [ ] §11 acceptance includes unrelated tenant/employee isolation, no automatic-quality claims, no event-replay promise, no secret bodies in version evidence, and accurate integration limitations.

Before implementing Task 1/2, update the spec with the explicit platform publication-intent directory and admin-safe model-options read endpoint; these are concrete supporting contracts, not a new business module. User already authorized continued work. External identity/deployment facts constrain real rollout, not independent implementation or test-principal verification.
