...
func newLifecycleTestApp(t *testing.T, persistenceEnabled bool, runError error) *fxtest.App {
	...
	return fxtest.New(t,
		...
		fx.Provide(func() pi.Runner {
			return runtimeFunc(func(context.Context, pi.RunRequest, pi.Reporter) (pi.RunResult, error) {
				return pi.RunResult{}, runError
			})
		}),
		fx.Provide(func() conversation.Runner {
			return conversationFunc(func(context.Context, conversation.RunRequest, pi.Reporter) (pi.RunResult, error) {
				return pi.RunResult{}, runError
			})
		}),
		fx.Supply(&config.Config{Conversation: config.ConversationConfig{Enabled: persistenceEnabled}}),
		fx.Supply(Prompt("test")),
		fx.Provide(func() pi.Reporter { return nil }),
