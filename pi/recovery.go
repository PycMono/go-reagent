...
// generate 执行一次 Thinking 或 Action 调用。遇到 Context Overflow 时走
// reactive 兜底：先 L1 裁剪，按判定规则直接重试或再尝试一次 L2 摘要，
// 只有 footprint 严格变小才重试原请求，且最多重试一次。
...
// 显式共享给主动（maybeCompact）与 reactive（generate）路径；
