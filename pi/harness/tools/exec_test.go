...
func TestExecToolStreamsForegroundStdoutAndStderrAndMarksNonzeroAsError(t *testing.T) {
	supervisor := newProcessSupervisorForTest(t, t.TempDir())
	toolRuntime := newTestToolRuntime(t, NewExecTool(supervisor))
	call := ai.ToolCall{ID: "exec-stream", Name: "exec", Arguments: execArguments(t, map[string]any{
		"command": toolHelperCommand("output-exit"),
		"yieldMs": 30_000,
	})}
	var events []testToolEvent
	result, err := toolRuntime.Execute(context.Background(), call, func(_ context.Context, event testToolEvent) {
	...
	if len(events) < 4 || events[0].Phase != testToolEventStart || events[len(events)-1].Phase != testToolEventEnd {
		t.Fatalf("events = %#v", events)
	}
	streams := make(map[string]string)
	for _, event := range events[1 : len(events)-1] {
		if event.Phase != testToolEventUpdate || event.Update == nil {
...
func TestExecToolExplicitBackgroundStartsWithStreamingGateClosed(t *testing.T) {
	...
	}, func(_ context.Context, event testToolEvent) {
		eventsMu.Lock()
		events = append(events, event)
		eventsMu.Unlock()
	})
	...
	if len(events) != 2 || events[0].Phase != testToolEventStart || events[1].Phase != testToolEventEnd {
...
func TestExecToolYieldClosesStreamingGateBeforeToolEnd(t *testing.T) {
	...
	eventsMu.Lock()
	defer eventsMu.Unlock()
	if len(events) != 3 || events[0].Phase != testToolEventStart || events[1].Phase != testToolEventUpdate || events[2].Phase != testToolEventEnd {
...
func TestExecToolPropagatesParentCancellationAsControlFlowError(t *testing.T) {
	...
	result, err := toolRuntime.Execute(ctx, ai.ToolCall{
		ID:        "exec-canceled",
		Name:      "exec",
...
func toolOutputText(t *testing.T, output ai.ToolOutput) string {
	t.Helper()
