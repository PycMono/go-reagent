...
func TestProcessToolRejectsLegacyFieldsUnknownActionsAndInvalidActionArguments(t *testing.T) {
	supervisor := newProcessSupervisorForTest(t, t.TempDir())
	toolRuntime := newTestToolRuntime(t, NewProcessTool(supervisor))
	tests := []struct {
...
func TestProcessToolNeverInvokesUpdateEmitter(t *testing.T) {
	...
	output, err := NewProcessTool(supervisor).Execute(context.Background(), processArguments(t, map[string]any{"action": "list"}), func(ai.ToolUpdate) {
		updates.Add(1)
	})
...
func executeProcessOutput(t *testing.T, tool *ProcessTool, input map[string]any) ai.ToolOutput {
	t.Helper()
