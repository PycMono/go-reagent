...
func (t *ExecTool) Execute(ctx context.Context, args json.RawMessage, emit ai.UpdateEmitter) (ai.ToolOutput, error) {
	input, err := decodeExecArgs(args)
	if err != nil {
		return ai.ToolOutput{}, err
	}
	if strings.TrimSpace(input.Command) == "" {
		return ai.ToolOutput{}, errors.New("command 不能为空")
	}
	...
	if timeoutSeconds < 1 || timeoutSeconds > maxExecTimeoutSeconds {
		return ai.ToolOutput{}, fmt.Errorf("timeout 必须在 1..%d 秒", maxExecTimeoutSeconds)
	}
	...
	if yieldMS < 0 || yieldMS > maxExecYieldMS {
		return ai.ToolOutput{}, fmt.Errorf("yieldMs 必须在 0..%d 毫秒", maxExecYieldMS)
	}
...
func (g *execStreamGate) emit(emit ai.UpdateEmitter, stream string, chunk []byte) {
...
func execOutput(snapshot ProcessSnapshot) ai.ToolOutput {
...
func finishedExecOutput(ctx context.Context, snapshot ProcessSnapshot) (ai.ToolOutput, error) {
