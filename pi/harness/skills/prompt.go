...
type PromptReport struct {
	// IncludedSkills 是实际写入提示词的 Skill 数量。
	IncludedSkills int
	// OmittedSkills 是因预算限制未写入提示词的 Skill 数量。
	OmittedSkills int
	// ShortenedDescriptions 是描述被缩短的 Skill 数量。
	ShortenedDescriptions int
	// Truncated 表示是否省略了 Skill 或缩短了描述。
	Truncated bool
}
...
// RenderPrompt 将当前 Skill 快照渲染成注入系统提示词的 XML 目录，
// 在技能数量和字符预算内优先保留身份信息，再按剩余空间补充描述。
func (s *Snapshot) RenderPrompt() (string, PromptReport) {
	skills := s.Skills()
	if len(skills) == 0 {
		return "", PromptReport{}
	}
