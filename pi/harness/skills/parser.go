...
func parseSkillMD(content []byte) (parsedSkill, error) {
	...
	osValues, ok := normalizedMetadataValues(metadata.Metadata.OpenClaw.OS)
	if !ok {
		return parsedSkill{}, newSkillParseError("skill_frontmatter_invalid", "SKILL.md Frontmatter 包含空环境要求")
	}
	requiredBins, ok := normalizedMetadataValues(metadata.Metadata.OpenClaw.Requires.Bins)
	if !ok {
		return parsedSkill{}, newSkillParseError("skill_frontmatter_invalid", "SKILL.md Frontmatter 包含空环境要求")
	}
	requiredEnv, ok := normalizedMetadataValues(metadata.Metadata.OpenClaw.Requires.Env)
	if !ok {
		return parsedSkill{}, newSkillParseError("skill_frontmatter_invalid", "SKILL.md Frontmatter 包含空环境要求")
	}
...
func normalizedMetadataValues(values []string) ([]string, bool) {
