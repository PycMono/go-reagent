...
// ErrInvalidRoot classifies an invalid or inaccessible Tool filesystem root.
var ErrInvalidRoot = errors.New("agent workspace invalid")
...
func NewWorkspace(lifecycle fx.Lifecycle, workDir Root) (*Workspace, error) {
	...
	if path == "" {
		return nil, fmt.Errorf("%w: workDir 不能为空", ErrInvalidRoot)
	}

	absPath, err := filepath.Abs(path)
	if err != nil {
		return nil, fmt.Errorf("%w: 解析工作区失败: %w", ErrInvalidRoot, err)
	}
	resolvedPath, err := filepath.EvalSymlinks(absPath)
	if err != nil {
		return nil, fmt.Errorf("%w: 解析工作区真实路径失败: %w", ErrInvalidRoot, err)
	}

	info, err := os.Stat(resolvedPath)
	if err != nil {
		return nil, fmt.Errorf("%w: 检查工作区失败: %w", ErrInvalidRoot, err)
	}
	if !info.IsDir() {
		return nil, fmt.Errorf("%w: workDir 必须是目录", ErrInvalidRoot)
	}

	root, err := os.OpenRoot(resolvedPath)
	if err != nil {
		return nil, fmt.Errorf("%w: 打开工作区失败: %w", ErrInvalidRoot, err)
	}
