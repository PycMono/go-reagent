...
func TestLoopRejectsUnmeteredOrIncorrectlyCostedResponse(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			provider := rawProviderFunc(func(context.Context, []ai.Message, []ai.ToolDefinition) (*ai.Message, error) {
				return &ai.Message{
...
type loopTestRuntime struct {
	...
	toolRuntime      pi.ToolRuntime
	factory          *harness.ContextBuilder
	enableThinking   bool
...
func newAgentLoopForTest(
	...
	return newAgentLoopRuntimeForTest(llmProvider, toolRuntimeWithRequiredRead{ToolRuntime: toolRuntime}, workDir, enableThinking)
}
...
func newAgentLoopRuntimeForTest(
	...
	return &loopTestRuntime{
		...
		factory:          harness.NewContextBuilder(harness.NewPromptComposer(workDir), workDir),
		enableThinking:   enableThinking,
		MaxParallelTools: 4,
	}
}

type toolRuntimeWithRequiredRead struct {
	pi.ToolRuntime
}

type recordingReporter struct {
	mu     sync.Mutex
	events []pi.AgentEvent
}

func (r *recordingReporter) Report(_ context.Context, event pi.AgentEvent) {
	r.mu.Lock()
	defer r.mu.Unlock()
	r.events = append(r.events, event)
}

func (r *recordingReporter) Events() []pi.AgentEvent {
	r.mu.Lock()
	defer r.mu.Unlock()
	return append([]pi.AgentEvent(nil), r.events...)
}
...
func TestLoopReportsEveryLifecycleEventWithoutAggregation(t *testing.T) {
	...
	runtime := newAgentLoopForTest(provider, toolRuntime, t.TempDir(), true)
...
func (r toolRuntimeWithRequiredRead) Definitions() []ai.ToolDefinition {
	definitions := r.ToolRuntime.Definitions()
...
func (r *fakeToolRuntime) Definitions() []ai.ToolDefinition {
	return append([]ai.ToolDefinition(nil), r.definitions...)
}

func (r *fakeToolRuntime) Execute(ctx context.Context, call ai.ToolCall, observer pi.ToolEventObserver) (pi.ToolResult, error) {
	r.mu.Lock()
	...
	if observer != nil {
		observer(ctx, pi.NewToolStart(call))
	}
	var result pi.ToolResult
	if result, ok := r.results[call.Name]; ok {
		if observer != nil {
			observer(ctx, pi.NewToolEnd(call, result))
		}
		return result, nil
	}
...
func (r *fakeToolRuntime) Calls() []ai.ToolCall {
	r.mu.Lock()
	defer r.mu.Unlock()
	return append([]ai.ToolCall(nil), r.calls...)
}

type controlledToolRuntime struct {
	definitions []ai.ToolDefinition
...
func (r *controlledToolRuntime) Definitions() []ai.ToolDefinition {
	return append([]ai.ToolDefinition(nil), r.definitions...)
}

func (r *controlledToolRuntime) Execute(ctx context.Context, call ai.ToolCall, observer pi.ToolEventObserver) (pi.ToolResult, error) {
	if observer != nil {
		observer(ctx, pi.NewToolStart(call))
	}
	select {
...
func canceledToolResult(call ai.ToolCall, err error) pi.ToolResult {
...
func TestAgentLoopPassesContextAndAvailableToolsToProvider(t *testing.T) {
	...
	toolRuntime := &fakeToolRuntime{definitions: []ai.ToolDefinition{
		{Name: "bash", Description: "execute a command", InputSchema: map[string]any{"type": "object"}},
	}}
	engine := newAgentLoopForTest(provider, toolRuntime, t.TempDir(), false)
...
func TestAgentLoopBuildsWorkspaceContextForEachRun(t *testing.T) {
	...
	toolRuntime := &fakeToolRuntime{definitions: []ai.ToolDefinition{{Name: "read", ParallelSafe: true}}}
	agentEngine := newAgentLoopForTest(provider, toolRuntime, workDir, false)
...
func TestAgentLoopRequiresReadWhenSkillsAreAvailable(t *testing.T) {
	...
	writeValidAgentWorkspace(workDir)
	agentEngine := newAgentLoopRuntimeForTest(provider, &fakeToolRuntime{}, workDir, false)
...
func TestAgentLoopAppendsToolObservationAndContinues(t *testing.T) {
	...
	toolRuntime := &fakeToolRuntime{results: map[string]pi.ToolResult{
...
func TestAgentLoopExecutesParallelSafeToolsConcurrentlyInCallOrder(t *testing.T) {
	...
	toolRuntime := &controlledToolRuntime{
		definitions: []ai.ToolDefinition{
	...
	agentEngine := newAgentLoopForTest(provider, toolRuntime, t.TempDir(), false)
...
func TestAgentLoopBoundsParallelTools(t *testing.T) {
	...
	toolRuntime := newControlledToolRuntime(toolCalls, definitions)
	agentEngine := newAgentLoopForTest(provider, toolRuntime, t.TempDir(), false)
	agentEngine.MaxParallelTools = 2

	done, markFinished := runEngineForTest(t, agentEngine, "read all", toolRuntime.gates)
	first := requireStartedCall(t, toolRuntime.started)
	second := requireStartedCall(t, toolRuntime.started)
	requireNoStartedCall(t, toolRuntime.started)

	close(toolRuntime.gates[first.ID])
	requireFinishedCall(t, toolRuntime.finished, first.ID)
	third := requireStartedCall(t, toolRuntime.started)
	if third.ID == first.ID || third.ID == second.ID {
		t.Fatalf("third started call = %q, want a queued call", third.ID)
	}

	closeAllGates(toolRuntime.gates)
	if err := <-done; err != nil {
		markFinished()
		t.Fatalf("Run() error = %v", err)
	}
...
func TestAgentLoopUsesExclusiveToolsAsBarriers(t *testing.T) {
	...
	toolRuntime := newControlledToolRuntime(toolCalls, definitions)
	toolRuntime.results["call-5"] = toolResult(ai.ToolCall{ID: "call-5", Name: "missing"}, `tool "missing" is not registered`, true)
	agentEngine := newAgentLoopForTest(provider, toolRuntime, t.TempDir(), false)

	done, markFinished := runEngineForTest(t, agentEngine, "run ordered batch", toolRuntime.gates)
	first := requireStartedCall(t, toolRuntime.started)
	second := requireStartedCall(t, toolRuntime.started)
	...
	requireFinishedCall(t, toolRuntime.finished, "")
	requireFinishedCall(t, toolRuntime.finished, "")
	writeCall := requireStartedCall(t, toolRuntime.started)
	if writeCall.Name != "write" {
		t.Fatalf("barrier call = %q, want write", writeCall.Name)
	}
	requireNoStartedCall(t, toolRuntime.started)

	close(toolRuntime.gates[writeCall.ID])
	requireFinishedCall(t, toolRuntime.finished, writeCall.ID)
	thirdRead := requireStartedCall(t, toolRuntime.started)
	...
	requireFinishedCall(t, toolRuntime.finished, thirdRead.ID)
	missing := requireStartedCall(t, toolRuntime.started)
	if missing.Name != "missing" {
		t.Fatalf("last phase call = %q, want missing", missing.Name)
	}
	close(toolRuntime.gates[missing.ID])
	requireFinishedCall(t, toolRuntime.finished, missing.ID)
...
func TestAgentLoopCompletesParallelSiblingsAfterToolError(t *testing.T) {
	...
	toolRuntime := newControlledToolRuntime(toolCalls, definitions)
	toolRuntime.results["call-1"] = toolResult(ai.ToolCall{ID: "call-1", Name: "read-1"}, "read failed", true)
	toolRuntime.results["call-2"] = toolResult(ai.ToolCall{ID: "call-2", Name: "read-2"}, "read ok", false)
	agentEngine := newAgentLoopForTest(provider, toolRuntime, t.TempDir(), false)

	done, markFinished := runEngineForTest(t, agentEngine, "read both", toolRuntime.gates)
	requireStartedCall(t, toolRuntime.started)
	requireStartedCall(t, toolRuntime.started)
	closeAllGates(toolRuntime.gates)
	if err := <-done; err != nil {
		markFinished()
...
func TestAgentLoopUsesSerialFallbackForNonpositiveParallelLimit(t *testing.T) {
	...
	done, markFinished := runEngineForTest(t, agentEngine, "read serially", toolRuntime.gates)
	first := requireStartedCall(t, toolRuntime.started)
	requireNoStartedCall(t, toolRuntime.started)
	close(toolRuntime.gates[first.ID])
	requireFinishedCall(t, toolRuntime.finished, first.ID)
	...
	close(toolRuntime.gates[second.ID])
	requireFinishedCall(t, toolRuntime.finished, second.ID)
...
func TestAgentLoopRejectsNilProviderResponse(t *testing.T) {
	...
	engine := newAgentLoopForTest(provider, &fakeToolRuntime{}, t.TempDir(), false)
...
func TestAgentLoopCarriesThinkingIntoActionContext(t *testing.T) {
	...
	engine := newAgentLoopForTest(provider, toolRuntime, t.TempDir(), true)
...
func TestAgentLoopRejectsNilThinkingResponse(t *testing.T) {
	provider := &fakeProvider{responses: []*ai.Message{nil}}
	engine := newAgentLoopForTest(provider, &fakeToolRuntime{}, t.TempDir(), true)
...
func TestAgentLoopRejectsToolCallsDuringThinking(t *testing.T) {
	...
	toolRuntime := &fakeToolRuntime{definitions: []ai.ToolDefinition{{Name: "bash"}}}
	engine := newAgentLoopForTest(provider, toolRuntime, t.TempDir(), true)
...
func toolResult(call ai.ToolCall, text string, isError bool) pi.ToolResult {
