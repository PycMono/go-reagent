...
func TestAgentRunPreservesPartialResultAndOfficialOpenAIError(t *testing.T) {
	...
	result, err := sdk.Run(context.Background(), reagent.RunRequest{
		RunID: "partial-run",
		Input: reagent.UserMessage("read the agent definition"),
	})
	if reagent.ErrorCodeOf(err) != reagent.ErrorCodeAIGeneration {
		t.Fatalf("error code = %q, error = %v", reagent.ErrorCodeOf(err), err)
	}
	...
	if callMessage.Role != reagent.RoleAssistant || len(callMessage.ToolCalls) != 1 || callMessage.ToolCalls[0].ID != "call-read" {
		t.Fatalf("tool call message = %#v", callMessage)
	}
	toolMessage := result.NewMessages[1]
	if toolMessage.Role != reagent.RoleTool || toolMessage.ToolCallID != "call-read" || toolMessage.IsError || len(toolMessage.Content) == 0 {
