...
type conversationFunc func(context.Context, conversation.RunRequest, engine.Reporter) (schema.RunResult, error)
...
func TestAgentRunnerUsesStatelessRuntimeWhenPersistenceDisabled(t *testing.T) {
	...
	runtime := runtimeFunc(func(context.Context, schema.RunRequest, engine.Reporter) (schema.RunResult, error) {
		runtimeCalled <- struct{}{}
		return schema.RunResult{}, nil
	})
	conversationRunner := conversationFunc(func(context.Context, conversation.RunRequest, engine.Reporter) (schema.RunResult, error) {
...
func TestAgentRunnerUsesConversationRunnerWhenPersistenceEnabled(t *testing.T) {
	...
	conversationRunner := conversationFunc(func(_ context.Context, request conversation.RunRequest, gotReporter engine.Reporter) (schema.RunResult, error) {
...
func TestAgentRunnerReportsConversationError(t *testing.T) {
	...
	runner, err := NewAgentRunner(
		runtimeFunc(func(context.Context, schema.RunRequest, engine.Reporter) (schema.RunResult, error) {
			return schema.RunResult{}, nil
		}),
		conversationFunc(func(context.Context, conversation.RunRequest, engine.Reporter) (schema.RunResult, error) {
...
func TestAgentRunnerStopCancelsSelectedConversationRunner(t *testing.T) {
	...
	runner, err := NewAgentRunner(
		...
		conversationFunc(func(ctx context.Context, _ conversation.RunRequest, _ engine.Reporter) (schema.RunResult, error) {
			<-ctx.Done()
			close(canceled)
...
func TestAgentRunnerReportsRuntimeError(t *testing.T) {
	...
	runner := newStatelessAgentRunner(t, runtimeFunc(func(context.Context, schema.RunRequest, engine.Reporter) (schema.RunResult, error) {
		return schema.RunResult{}, want
...
func TestAgentRunnerStopCancelsAndWaitsForRuntime(t *testing.T) {
	canceled := make(chan struct{})
	runner := newStatelessAgentRunner(t, runtimeFunc(func(ctx context.Context, _ schema.RunRequest, _ engine.Reporter) (schema.RunResult, error) {
		<-ctx.Done()
		close(canceled)
		return schema.RunResult{}, ctx.Err()
	}), config.Prompt("test"), nil)
...
func newStatelessAgentRunner(t *testing.T, runtime engine.AgentRuntime, prompt config.Prompt, reporter engine.Reporter) *AgentRunner {
	t.Helper()
